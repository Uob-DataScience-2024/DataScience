...
class VisualTest(unittest.TestCase):
    ...
    def test_new_data(self):
        ...
        dt = dts[1]
        draw_by_time_df(image, merge.game.copy(), dt, home_visitor[0])
        # dpi = 100
        image, bottom_y = draw_background(image, f"{home_visitor[0]} vs {home_visitor[1]} - {str(game)}", (209, 222, 233), (242, 149, 89))
        draw_status_df(image, merge.game[merge.game['gameId'] == game], dt, home_visitor[0], bottom_y, targetY='event')
        plt.figure(figsize=(24, 11.06))
        plt.imshow(image)
        plt.axis('off')
        plt.show()
    ...
    def test_video_new_data(self):
        ...
        for dt in tqdm(dts):
            ...
            # draw datetime
            image, bottom_y = draw_background(image, f"{home_visitor[0]} vs {home_visitor[1]} - {str(game)}", (209, 222, 233), (242, 149, 89))
            draw_status_df(image, data, dt, home_visitor[0], bottom_y, targetY='event')
            cv2.putText(image, dt.strftime('%Y-%m-%d %H:%M:%S'), (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (32, 44, 57), 2)
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            result.append((np.array(image, dtype=np.uint8), ms_interval))
