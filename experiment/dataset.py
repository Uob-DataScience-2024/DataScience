...
class DatasetPffBlockTypeScore(Dataset):
    def __init__(self, data_dir, pff_filename='pffScoutingData.csv', tracking_filename='week{}.csv', play_data_filename="plays.csv", games_file="games.csv", cache=False,
        ...
        self.build_category_table()
        self.games_data = GameData(os.path.join(self.data_dir, games_file))
    ...
    def __getitem__(self, idx):
        ...
        category_table = {}
        results = []
        game_id = data['gameId'].iloc[0]
        home_visitor = self.home_visitor[game_id]
        self.category_table['team'] = home_visitor
        for name in self.x_col:
            if name in self.x_category_col:
                if name in self.x_self_category_col:
                    results.append(data[name].astype('category').cat.codes)
        ...
        # label = [self.category_table[self.y_col].index(i) if type(i) == str else 0 for i in raw_label]
        results = torch.tensor(np.array(results).T, dtype=torch.float32)
        label = torch.tensor(np.array(raw_label))
        # results = self.transform(results)
        return results, F.one_hot(label.long(), num_classes=55).to(torch.float32)
